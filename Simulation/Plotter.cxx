...
void Plotter(const std::vector<double> &Exs, const std::string &beam, const std::string &target,
             const std::string &light, double T1, double pressure)
{
    ...
    {
        TString fileName{TString::Format("../PostAnalysis/Input/Simu_%s_%dmbar.root", light.c_str(), pressure)};
        ...
        auto hEx{df.Histo1D(HistConfig::ChangeTitle(HistConfig::Ex, TString::Format("%s(%s, %s) Ex = %.2f", beam.c_str(), target.c_str(), light.c_str(), Ex)), "Eex", "weight")};
        auto hKin{df.Histo2D(HistConfig::ChangeTitle(HistConfig::KinSimu, TString::Format("%s(%s, %s) Ex = %.2f", beam.c_str(), target.c_str(), light.c_str(), Ex)), "theta3Lab", "EVertex", "weight")};
