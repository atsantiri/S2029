...
void beamEmittance()
{
    ...
    auto dff {df.Filter([](ActRoot::ModularData& m) { return m.Get("GATCONF") == 2; }, {"ModularData"})
    ...
    hEnd->DrawClone("colz");
    c->cd(3);
    auto hThetaXZ {dff.Histo1D({"hThetaXZ", "XZ Angle;;", 200, -10, 10}, "thetaXZ")};
    hThetaXZ->DrawClone();
    c->cd(4);
    auto hThetaXY {dff.Histo1D({"hThetaXY", "XY Angle;;", 200, -10, 10}, "thetaXY")};
    hThetaXY->DrawClone();
    // c->cd(4);
    // auto hYThetaXY {dff.Histo2D({"hYThetaXY", "Y vs #theta_{XY};Y [pad];#theta_{XY} [#circ]", 128, 0, 128, 150, -10, 10},"Y1","thetaXY")};
