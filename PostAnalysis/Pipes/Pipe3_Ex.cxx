...
void Pipe3_Ex(const std::string &beam, const std::string &target, const std::string &light)
{
    ...
    def = def.Define("EBeam",
                     ...
                     {"MergerData"})
              .Define("ECM", [&](double EBeam)
                      { return (EBeam * lab_to_com); }, {"EBeam"})
              .Define("ECN", [&](double Ecm)
                      { return Ecm + qval; }, {"ECM"})
              .DefineSlot("RecEx",
              ...
              .DefineSlot("RecEBeam",
                          ...
                          {"MergerData", "EVertex"})
              .Define("RecECM", [&](double rec_EBeam)
                      { return lab_to_com * rec_EBeam; }, {"RecEBeam"})
              .Define("RecECN", [&](double rec_ECM)
                      { return rec_ECM + qval; }, {"RecECM"});
    ...
    // Ex Si
    auto hExESi{def.Filter([](ActRoot::MergerData &m)
                           { return m.fLight.GetNLayers() == 1; }, {"MergerData"})
                    .Histo2D(HistConfig::ExESi, "fLight.fEs", "RecEx")};
    auto hRecExSil{def.Filter([](ActRoot::MergerData &m)
                              { return m.fLight.GetNLayers() == 1; }, {"MergerData"})
                       .Histo1D(HistConfig::Ex, "RecEx")};
    hRecExSil->SetTitle("Reconstructed Ex with silicons");
    auto hRecExSilRPx{def.Filter([](ActRoot::MergerData &m)
                                 { return m.fLight.GetNLayers() == 1; }, {"MergerData"})
    ...
    auto hRecExL1{def.Filter([](ActRoot::MergerData &m)
                             { return m.fLight.IsFilled() == false; }, {"MergerData"})
                      .Histo1D(HistConfig::Ex, "RecEx")};
    hRecExL1->SetTitle("Reconstructed Ex with L1");
    auto hRecExL1RPx{def.Filter([](ActRoot::MergerData &m)
                                { return m.fLight.IsFilled() == false; }, {"MergerData"})
                         .Histo2D(HistConfig::ExRPx, "fRP.fCoordinates.fX", "RecEx")};
    ...
    // Ecm
    auto hEcnSi{def.Filter([](ActRoot::MergerData &m)
                           { return m.fLight.GetNLayers() == 1; }, {"MergerData"})
                    .Histo1D(HistConfig::ECN, "ECN")};

    auto hRecEcnSi{def.Filter([](ActRoot::MergerData &m)
                              { return m.fLight.GetNLayers() == 1; }, {"MergerData"})
    ...
    auto hEcnL1{def.Filter([](ActRoot::MergerData &m)
                           { return m.fLight.IsFilled() == false; }, {"MergerData"})
                    .Histo1D(HistConfig::ECN, "ECN")};
    auto hRecEcnL1{def.Filter([](ActRoot::MergerData &m)
                              { return m.fLight.IsFilled() == false; }, {"MergerData"})
                       .Histo1D(HistConfig::ECN, "RecECN")};
