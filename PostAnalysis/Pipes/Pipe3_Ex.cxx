              ...
              const std::string gateOn)
{
    ...
    // else if(light == "4He")
    fIn = TString::Format("./Outputs/tree_ESil_BSP_%s_%s_%s_%.2f_%s.root", beam.c_str(), target.c_str(), light.c_str(),
                          EBeamIni, gateOn.c_str());
    ...
    auto* c34 {new TCanvas("c34", "Pipe 3 Canvas 4: ECN", 900, 600)};
    // hRecEcnSi->DrawClone();
    ...
    // light.c_str())};
    auto outfile {TString::Format("./Outputs/tree_ex_%s_%s_%s_%.2f_%s.root", beam.c_str(), target.c_str(),
