...
void Pipe1_PID(const std::string& beam, const std::string& target, const std::string& light, const double EBeamIni,
               const std::string& gateOn)
{
    ...
    if(listOfCuts.size())
    {
        ...
        auto name {TString::Format("./Outputs/tree_pid_%s_%s_%s_%.2f_%s.root", beam.c_str(), target.c_str(),
    ...
    if(gateOn == "l1")
    {
        ...
        gtheo->Draw("l");
        auto* gtheo2 {
            ActPhysics::Kinematics(TString::Format("%s(p,4He)@65.2", beam.c_str()).Data()).GetTheta3vs4Line()};
